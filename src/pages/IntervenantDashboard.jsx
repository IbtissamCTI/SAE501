...
import {
  ...
  Grid, List, AlertCircle, Trash2, Briefcase, TrendingUp
} from "lucide-react";
  ...
  useEffect(() => {
    const chargerDonnees = async () => {
      try {
        ...
        // On récupère le profil local
        setInfosProf(prev => ({ ...prev, ...user }));

        // ✅ Appels API réels vers ton Docker
        ...
        // On définit la prochaine session (la plus proche)
    ...
    <div className="min-h-screen bg-[#050507] text-white p-6 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-3xl font-bold tracking-tighter uppercase italic">Espace Intervenant</h1>
            <p className="text-gray-500 text-sm">Bonjour, {infosProf.prenom} {infosProf.nom}</p>
          </div>
          <div className="flex gap-4">
            <button onClick={() => setActiveTab("planning")} className="p-3 bg-[#1F1F23] rounded-2xl hover:bg-[#2A2A30] transition">
              <Calendar size={20} className="text-gray-400" />
            </button>
            <button onClick={() => setActiveTab("donnees")} className="p-3 bg-[#5B4DFF]/10 border border-[#5B4DFF]/20 rounded-2xl text-[#5B4DFF] hover:bg-[#5B4DFF]/20 transition">
              <User size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-8 mb-8 border-b border-[#1F1F23]">
          {['dashboard', 'planning', 'donnees'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} 
              className={`pb-4 text-xs font-bold uppercase tracking-widest transition-all ${activeTab === tab ? 'text-[#5B4DFF] border-b-2 border-[#5B4DFF]' : 'text-gray-500'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === "dashboard" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* PROCHAINE SESSION (Connecté au Back) */}
            <div className="md:col-span-2 bg-[#0A0A0C] border border-[#1F1F23] p-8 rounded-3xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-8 text-[#5B4DFF]/10 group-hover:text-[#5B4DFF]/20 transition-all">
                <Clock size={120} />
              </div>
              <h2 className="text-gray-500 text-xs font-bold uppercase mb-4 flex items-center gap-2">
                <div className="w-2 h-2 bg-[#5B4DFF] rounded-full animate-pulse" /> Prochaine Session
              </h2>
              {prochaineSession ? (
                <>
                ...
                <p className="text-gray-500 italic">Aucune session planifiée prochainement.</p>
              )}
            </div>
            ...
            <div className="bg-[#5B4DFF] p-8 rounded-3xl shadow-[0_20px_40px_rgba(91,77,255,0.2)]">
                ...
                <div className="mt-8 pt-8 border-t border-white/10 flex justify-between items-end">
                    <div><p className="text-white/60 text-[10px] uppercase font-bold">Total Heures</p><p className="text-2xl font-bold">{statsDonnees.totalHeures}h</p></div>
                    <TrendingUp size={32} />
                </div>
            </div>
          </div>
        ...
        {/* CONTENU : PLANNING (Sessions attitrées) */}
        {activeTab === "planning" && (
            ...
            {planningProf.length > 0 ? planningProf.map(session => (
              <div key={session.id} className="bg-[#0A0A0C] border border-[#1F1F23] p-6 rounded-2xl flex justify-between items-center hover:border-[#5B4DFF]/50 transition cursor-pointer">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h4 className="font-bold text-lg italic uppercase">{session.formation?.titre}</h4>
                    <p className="text-gray-500 text-sm flex items-center gap-2"><MapPin size={14}/> {session.lieu} - {session.salle}</p>
                  </div>
                </div>
                <button className="p-3 bg-[#1F1F23] rounded-xl hover:bg-[#5B4DFF] hover:text-white transition group">
                  <ChevronRight size={20} className="group-hover:translate-x-1 transition" />
                </button>
              </div>
            )) : <p className="text-gray-500 italic">Votre planning est vide pour le moment.</p>}
          </div>
        )}
        ...
        {activeTab === "donnees" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-2 bg-[#0A0A0C] border border-[#1F1F23] p-8 rounded-3xl">
              ...
              <form onSubmit={handleSaveProfile} className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div><label className="text-[10px] uppercase font-bold text-gray-500 mb-2 block">Nom</label><input name="nom" value={infosProf.nom} onChange={handleInputChange} className="w-full bg-[#1F1F23] border border-transparent focus:border-[#5B4DFF] p-4 rounded-xl outline-none" /></div>
                  <div><label className="text-[10px] uppercase font-bold text-gray-500 mb-2 block">Prénom</label><input name="prenom" value={infosProf.prenom} onChange={handleInputChange} className="w-full bg-[#1F1F23] border border-transparent focus:border-[#5B4DFF] p-4 rounded-xl outline-none" /></div>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div><label className="text-[10px] uppercase font-bold text-gray-500 mb-2 block">Email</label><input name="email" value={infosProf.email} onChange={handleInputChange} className="w-full bg-[#1F1F23] border border-transparent focus:border-[#5B4DFF] p-4 rounded-xl outline-none" /></div>
                  <div><label className="text-[10px] uppercase font-bold text-gray-500 mb-2 block">IBAN (Paiement)</label><input name="iban" value={infosProf.iban} onChange={handleInputChange} className="w-full bg-[#1F1F23] border border-transparent focus:border-[#5B4DFF] p-4 rounded-xl outline-none" /></div>
                </div>
            ...
            <div className="space-y-6">
              <div className="bg-[#1F1F23]/50 p-6 rounded-3xl border border-[#1F1F23]">
                <h4 className="font-bold mb-4 uppercase text-xs text-[#5B4DFF]">Résumé activité</h4>
                <div className="space-y-4">
                  <div className="flex justify-between">
                    <span className="text-gray-500 text-sm">Prochaine séance</span>
                    <span className="text-white text-sm font-bold">{statsDonnees.prochainEvenement}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500 text-sm">Total sessions</span>
                    <span className="text-white text-sm font-bold">{statsDonnees.totalSessions}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {notification && (
          <div className="fixed bottom-10 right-10 bg-[#5B4DFF] text-white px-8 py-4 rounded-2xl font-bold animate-in slide-in-from-right shadow-2xl">
            {notification}
            <button onClick={() => setNotification(null)} className="ml-4 opacity-50 hover:opacity-100">X</button>
          </div>
        )}
